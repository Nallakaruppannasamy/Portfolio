  ...
  return (
    ...
    >
      <a
        href={href}
        onClick={onClick}
        className={`relative px-4 py-2 text-[10px] font-mono uppercase tracking-[0.2em] transition-colors duration-300 flex items-center group ${isActive ? 'text-cyan-400' : 'text-gray-500 hover:text-white'
          }`}
      >
...
export default function Navigation() {
  ...
  const links = [
    ...
    { name: 'Education', href: '#education', id: '06' },
    ...
    <>
      ...
      <div className="fixed left-8 top-1/2 -translate-y-1/2 z-60 hidden xl:flex flex-col gap-6 items-center">
        ...
        {links.map((link) => (
          <span
            key={link.id}
            className={`font-mono text-[10px] transition-all duration-500 ${activeSection === link.href.substring(1) ? 'text-cyan-400 scale-150 font-bold' : 'text-gray-700'
              }`}
          >
            {link.id}
          </span>
      ...
      <nav className={`fixed top-0 w-full z-100 transition-all duration-500 border-b ${isScrolled ? 'bg-[#050505]/80 backdrop-blur-xl border-white/5 py-3' : 'bg-transparent border-transparent py-8'
        } px-8 md:px-16`}>
        ...
        <div className="max-w-9xl mx-auto flex justify-between items-center relative z-10">
